...
export default function PerformaInvoicesPage() {
  ...
  const handleEdit = (id: string) => {
    router.push(
      `/dashboard/invoice-generation?id=${id}&type=${invoiceType}&returnTo=performa-invoices`
    );
  };

  // Handle converting performa invoice to tax invoice
  const handlePlay = (id: string, invoiceNumber: string) => {
    router.push(
      `/dashboard/invoice-generation?id=${id}&type=${invoiceType}&convert=true&source=${invoiceNumber}&returnTo=tax-invoices`
    );
  };
  ...
  useEffect(() => {
    const fetchInvoices = async () => {
      ...
      try {
        ...
        // Try to determine the correct document type code for performa invoices
        let documentTypeCode = "performa";

        // Check if there are any document types that might be performa invoices
        if (docTypes && docTypes.length > 0) {
          const possiblePerformaTypes = docTypes.filter((item) => {
            const code = item.document_type_code?.toLowerCase() || "";
            return (
              code.includes("performa") ||
              code.includes("proforma") ||
              code === "pfi"
            );
          });

          if (possiblePerformaTypes.length > 0) {
            documentTypeCode = possiblePerformaTypes[0].document_type_code;
          }
        }
  ...
  return (
    <div className="container mx-auto py-6">
      <Card>
        <CardHeader>
          <CardTitle>
            {invoiceType === "purchase" ? "Purchase Order (Quotation)" : "Sales Quotations"}
          </CardTitle>
          <CardDescription>
            Manage your {invoiceType === "purchase" ? "purchase order quotations" : "sales quotations"} with search, sort, and
            filter options.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="p-4 text-center">
              <p>No {invoiceType === "purchase" ? "purchase order quotations" : "sales quotations"} found.</p>
            </div>
          ) : (
            <>
